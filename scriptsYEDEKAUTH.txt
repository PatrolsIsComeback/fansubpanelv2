...
elements.loginForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // Firestore'dan kullanıcı belgesini kontrol et
        const userDoc = await db.collection('users').doc(user.uid).get();
        if (!userDoc.exists) {
            // Kullanıcı belgesi yoksa, oturumu kapat ve uyarı ver
            await auth.signOut();
...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        if (userDoc.exists) {
            ...
            await auth.signOut();
            showModal('Hesabınız yetkilendirilmemiş. Lütfen yönetici onayı bekleyin.');
        }
    } else {
